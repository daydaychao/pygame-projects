...
class Hero(pygame.sprite.Sprite):
    def __init__(self, image_paths, position=(375, 520), **kwargs):
        pygame.sprite.Sprite.__init__(self)
        self.images_right = [pygame.image.load(
            path) for path in image_paths[:5]]
        self.images_left = [pygame.image.load(
            path) for path in image_paths[5:]]
    ...
    def move(self, screensize, direction):
        ...
        if direction != self.diretion:
            self.images = self.images_left.copy(
            ) if direction == 'left' else self.images_right.copy()
            self.image = self.images[0]
